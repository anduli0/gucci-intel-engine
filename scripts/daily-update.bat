...
rem Uses content dates (see special_age.py) — filesystem mtime is reset by every
rem publish/checkout, which made this guard never fire on the PC path.
python scripts\special_age.py >> %LOG% 2>&1
if errorlevel 1 (
